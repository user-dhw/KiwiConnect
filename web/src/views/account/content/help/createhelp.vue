<template>
	<div class="edit-page">
		<h2>{{ isEdit ? 'Edit Q&A' : 'Create Q&A' }}</h2>
		<el-form :model="form" label-width="120px">
			...
			<el-form-item>
				...
				<el-button @click="router.push('/admin/createhelplist')"
					>Cancel</el-button
				>
			</el-form-item>
		</el-form>
...
const submit = async () => {
	if (
		!form.value.help_title ||
		!form.value.help_lable ||
		!form.value.help_content
	) {
...
const loadDetails = async () => {
	...
	dynamicTags.value = data.help_tag
		? String(data.help_tag).split(',').filter(Boolean)
		: []
}
